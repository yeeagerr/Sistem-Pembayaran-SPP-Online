...
if (empty($_SESSION['iduser'])) {
	...
	if (isset($_REQUEST['submit'])) {
		...
		if (($submit == 'simpan') and isset($_REQUEST['nis'])) {
			$nis = $_REQUEST['nis'];
			$sql = mysqli_query($conn, "INSERT INTO kelas VALUES('$kelas','$tapel','$nis')");
		}

		//proses hapus siswa dari kelas
		if (($submit == 'hapus') and isset($_REQUEST['nis'])) {
			$nis = $_REQUEST['nis'];
			$qsiswa = mysqli_query($conn, "DELETE FROM kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel' AND nis='$nis'");
		}
		...
		$qsiswa = mysqli_query($conn, "SELECT nis,nama FROM siswa WHERE idprodi='$idprodi' AND nis NOT IN (SELECT nis FROM kelas ) ORDER BY nis");
		...
		$qkelas = mysqli_query($conn, "SELECT nis FROM kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel'");
		if (mysqli_num_rows($qkelas) > 0) {
			$no = 1;
			while (list($nis) = mysqli_fetch_array($qkelas)) {
				echo '<tr><td>' . $no . '</td>';
				echo '<td>' . $nis . '</td>';
				$qsiswa = mysqli_query($conn, "SELECT nama FROM siswa WHERE nis='$nis'");
			...
			<div class="form-group">
				...
				<div class="col-sm-2">
					...
					<select name="tapel" class="form-control">
						<?php
						$qtapel = mysqli_query($conn, "SELECT tapel FROM tapel ORDER BY tapel DESC");

	...
	*/
	...
	if (isset($_REQUEST['submit'])) {
		...
		if ($submit == 'bayar') {
			...
			$qbayar = mysqli_query($conn, "INSERT INTO pembayaran VALUES('$kls','$nis','$bln','$tgl','$jml')");
		...
		if ($submit == 'hapus') {
			...
			$qbayar = mysqli_query($conn, "DELETE FROM pembayaran WHERE kelas='$kls' AND nis='$nis' AND bulan='$bln'");
		...
		$qsiswa = mysqli_query($conn, "SELECT * FROM siswa WHERE nis='$nis'");
...
?>
		<form class="form-inline" role="form" method="post" action="./admin.php?hlm=bayar">
			...
			<div class="form-group">
				...
				<select name="kls" class="form-control" id="kls">
					<?php
					$qkelas = mysqli_query($conn, "SELECT kelas,th_pelajaran FROM kelas WHERE nis='$nis'");
		...
		$qbayar = mysqli_query($conn, "SELECT kelas,bulan,tgl_bayar,jumlah FROM pembayaran WHERE nis='$nis' ORDER BY tgl_bayar DESC");
		...
		<form class="form-horizontal" role="form" method="post" action="./admin.php?hlm=bayar">
			<div class="form-group">
				<label for="nis" class="col-sm-2 control-label">Nomor Induk Mahasiswa</label>
				<div class="col-sm-3">
			...
			<div class="form-group">
				<div class="col-sm-offset-2 col-sm-3">
					<button type="submit" name="submit" class="btn btn-default" value="Cari">Cari</button>
				</div>
			</div>
		</form>
<?php
